...
*Education attainment for more than primary school
bysort id : replace Ed3max=Maxeduc3[_n-1] if generation[_n]==2 & generation[_n-1]==1
bysort id : replace Ed3max=Ed3max[_n-1] if generation[_n]==generation[_n-1]
bysort id : replace Ed3max=Maxeduc3[_n-1] if generation[_n]==1 & generation[_n-1]==0
bysort id : replace Ed3max=Ed3max[_n-1] if generation[_n]==generation[_n-1]
bysort id : replace Ed3max=Maxeduc3[_n-1] if generation[_n]==0 & generation[_n-1]==-1
bysort id : replace Ed3max=Ed3max[_n-1] if generation[_n]==generation[_n-1]	
bysort id : replace Ed3max=Maxeduc3[_n-1] if generation[_n]==-1 & generation[_n-1]==-2
bysort id : replace Ed3max=Ed3max[_n-1] if generation[_n]==generation[_n-1]
...
save Data_Intergenerational_mobility.dta,replace
